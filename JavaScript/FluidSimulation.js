const state = {
  ...
  g: new Float32Array(PARTICLE_COUNT), // 'nearness' to neighbor
  mesh:[]
};
...
let dt = 0.0166;
let GRID_CELLS = 54;
//Pass 1 - apply forces and advance particle based on velocity
for( let i=0; i < PARTICLE_COUNT; ++i ){

  //Update old position
  state.oldX[i] = state.x[i];
  state.oldY[i] = state.x[i];
...
//Pass 2 - Calculate densities and near-Densities
...
const getNeighboursWithGradients = i => {
  ...
  for( let k = 0; k < results.length; ++k ) {
    ...
    state.g[n] = g; //Store the getNeighbourWithGradient
...
const updatePressure = (i, neighbours) => {
  ...
  for( let k=0; k < neighbours.length; k++){
    const g = state.g[neighbors[k]]; //Get g for this neighbor
    density += g*g;
    nearDensity += g*g*g;
  }
...
const relax = (i, neighbours, dt) => {
  ...
  for ( let k=0; k < neighbours.length; k++ ){
    ...
    //Newtons Third Law of Motion - Adjusted
    state.x[i] += d[0] * -0.5;
    state.y[i] += d[1] * -0.5;
...
for( let i=0; i < PARTICLE_COUNT; ++i) {
  const neighbours = getNeighbourWithGradients(i);
...
//Pass 3 - Contain the particles within a boundary
